...
* **Zero Disk Footprint**: All visualization and QC report files are created temporarily. Upon closing TGV, an automated cleanup routine (guaranteed by the application's `finally` block) removes all temporary files from the user's hard drive [3].
* **Lightweight & Portable**: Developed without heavy external libraries (no Pandas, NumPy, or Jinja2). It can be shared as a standalone, zero-install executable for Windows, or run as a standard Python script on Linux/macOS.

---

### 🚀 Usage

**TGV** adapts to your environment and can be run in two different ways:

#### Option A: On Windows (Standalone Executable)
Designed for clinicians and biologists on Windows workstations.
1. Download the standalone **`TGV.exe`** executable from the *Releases* tab of this GitHub repository.
2. Double-click the executable to launch the application.
*No Python installation or library configuration is required.*

#### Option B: On Linux / macOS (Command Line)
Designed for bioinformaticians or server environment usage.
...
## 💻 Déploiement & Compilation / Deployment (Developer)
...
## 📝 Licence & Auteurs / License & Authors
